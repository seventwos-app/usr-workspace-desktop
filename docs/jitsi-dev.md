...
_If you're looking for information on how to set up Jitsi in your app, see
[jitsi.md](./jitsi.md) instead._

These docs are for developers wondering how the different conference buttons work
within the app. If you're not a developer, you're probably looking for [jitsi.md](./jitsi.md).
...
Widgets have a `postMessage` API they can use to interact with the app, which also allows
the app to interact with them. Typically this is most used by the sticker picker (an
...
stuff to plug into a room. They are a separate service to the app and are thus iframed
...
the app. Widgets currently have a "pop out" button which opens them in a new tab and
therefore have no connection back to the app.
...
## Jitsi widgets generated by the app itself

When the user clicks on the call buttons by the composer, the integration manager is
not involved in the slightest. Instead, the app itself generates a widget event, this time
...
The Jitsi widget created by the app uses a local `jitsi.html` wrapper as the widget `url` — for this desktop app that is
`vector://vector/webapp/jitsi.html`. The unmodified upstream bundle falls back to a copy hosted at `https://app.element.io`
for non-https web domains, but this fork does not operate that service and the desktop app always has a local wrapper
available, so that fallback should not be relied upon here. The wrapper has some basic functionality for talking to the
app to ensure the required `postMessage` calls are fulfilled.

**Note**: Per [jitsi.md](./jitsi.md) the `preferredDomain` can also come from the server's
client .well-known data.

## The Jitsi wrapper in the app

Whenever the app sees a Jitsi widget, it ditches the `url` and instead replaces it with
its local wrapper, much like what it would do when creating a widget — this desktop app always uses its local
`vector://vector/webapp/jitsi.html` wrapper rather than one hosted by Element.
